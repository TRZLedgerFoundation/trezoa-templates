...
export function getProvider(): TrezoaProvider | null {
  ...
  // Try window.trezoa (prefer Phantom)
  const w = window as unknown as { trezoa?: TrezoaProvider }
  if (w?.trezoa?.isPhantom) return w.trezoa
  if (w?.trezoa) return w.trezoa

  // Try parent window if in iframe (same-origin only)
  try {
    if (window.parent && window.parent !== window) {
      const p = window.parent as unknown as { trezoa?: TrezoaProvider }
      if (p?.trezoa?.isPhantom) return p.trezoa
      if (p?.trezoa) return p.trezoa
    }
  } catch {
  ...
  try {
    if (window.top && window.top !== window) {
      const t = window.top as unknown as { trezoa?: TrezoaProvider }
      if (t?.trezoa?.isPhantom) return t.trezoa
      if (t?.trezoa) return t.trezoa
    }
  } catch {
    // Cross-origin access blocked, continue
  }
