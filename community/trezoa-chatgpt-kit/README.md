...
TREZOA_PRIVATE_KEY=your_base58_private_key_here
...
TREZOA_RPC_URL=https://api.mainnet-beta.trezoa.com
...
When using external wallet mode (`externalWallet = true` in `lib/trezoa-config.ts`), this application implements the [x402 payment protocol](https://github.com/payainetwork/x402-trezoa) for API monetization. Each transfer requires a $0.001 USDC payment fee from the user's wallet before execution.
...
TREZOA_RPC_URL=https://your-custom-rpc-endpoint.com
```
