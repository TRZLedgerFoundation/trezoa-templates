...
// See https://trezoa.stackexchange.com/a/9860/98 for more details.
...
>({
  ...
  fetchMethod: async (_key, _value, { context }) => {
    ...
    context.log.debug(`[getTrezoaCachedBlockhash] cache write blockhash ${latestBlockhash.blockhash}`)
    return { ...latestBlockhash, cachedAt: Date.now() }
  },
})

export async function getTrezoaCachedBlockhash(context: ApiContext) {
